...
def test_g7_writes_artifacts_and_pass(tmp_path: Path):
    ...
    run_dir = runs / "2099-01-01_0010"
    run_dir.mkdir()
    (run_dir / "00_USER_REQUEST.md").write_text("Build pipeline\n", encoding="utf-8")

    meta = RunMeta(run_id="2099-01-01_0010", created_at=now_seoul().isoformat())
    ...
    assert (run_dir / "G7_DECISION.md").exists()
    assert (run_dir / "G7_OUTPUT.json").exists()
    assert (run_dir / "G7_META.json").exists()

    out = json.loads((run_dir / "G7_OUTPUT.json").read_text(encoding="utf-8"))
    assert out["gate"] == "G7"
    assert out["baseline_recommendation"]["recommend_update"] in (True, False)
