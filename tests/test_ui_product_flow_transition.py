...
from src.ui.product_flow_transition import read_product_flow_transition_view_model


def _working_save() -> WorkingSaveModel:
    return WorkingSaveModel(
        meta=WorkingSaveMeta(format_version="1.0.0", storage_role="working_save", working_save_id="ws-001", name="Transition Draft"),
...
def test_product_flow_transition_prioritizes_approval_to_commit_for_reviewed_working_save() -> None:
    vm = read_product_flow_transition_view_model(
        _working_save(),
    ...
    assert vm.transition_status == "actionable"
    assert vm.current_transition_id == "approval_to_commit"
    assert vm.recommended_transition_id == "approval_to_commit"
    assert vm.next_action_id == "commit_snapshot"
    assert any(item.transition_id == "approval_to_commit" and item.crossable for item in vm.transitions)


def test_product_flow_transition_marks_commit_to_run_live_when_execution_is_running() -> None:
    vm = read_product_flow_transition_view_model(_working_save(), execution_record=_run("running"))

    assert vm.source_role == "working_save"
    assert vm.transition_status == "live"
    assert vm.live_transition_id == "commit_to_run"
    assert any(item.transition_id == "commit_to_run" and item.live for item in vm.transitions)


def test_product_flow_transition_prefers_followthrough_after_completed_run_history() -> None:
    vm = read_product_flow_transition_view_model(_working_save(), execution_record=_run("completed"))

    assert vm.source_role == "working_save"
    assert vm.transition_status in {"actionable", "terminal"}
    assert vm.recommended_transition_id == "run_to_followthrough"
    assert any(item.transition_id == "run_to_followthrough" and (item.crossable or item.crossed) for item in vm.transitions)


def test_product_flow_transition_marks_approval_to_commit_blocked_when_precheck_blocks_commit_boundary() -> None:
    vm = read_product_flow_transition_view_model(
        _working_save(),
        validation_report=_validation_report(blocking=1),
        session_state_card=_session_card(),
        intent=_intent(),
        patch_plan=_patch(),
        precheck=_precheck("blocked"),
        preview=_preview(),
        approval_flow=_approval(),
    )

    transition = next(item for item in vm.transitions if item.transition_id == "approval_to_commit")
    assert vm.transition_status == "blocked"
    assert transition.transition_status == "blocked"
    assert transition.open_requirement_id == "commit_snapshot"
