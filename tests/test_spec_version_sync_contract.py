...
def _extract_active_spec_doc_paths_from_blueprint(blueprint_text: str) -> list[str]:
    ...
    """
    m = re.search(r"^##\s*2\.\s*Active Specifications\s*$([\s\S]*?)(?=^##\s|\Z)", blueprint_text, re.MULTILINE)
    if not m:
        raise AssertionError("BLUEPRINT missing '## 2. Active Specifications' section")
    block = m.group(1)
    paths = re.findall(r"^\s*-\s+([^\s]*docs/specs/[^\s]+\.md)\s*$", block, re.MULTILINE)
    # Normalize any leading './' etc.
    norm = []
    for p in paths:
        p = p.strip()
        ...
        norm.append(p)
    if not norm:
        raise AssertionError("No active spec doc paths found in BLUEPRINT section")
    return norm
...
def test_spec_versions_are_complete_and_synced_for_active_specs():
    ...
    active_docs = _extract_active_spec_doc_paths_from_blueprint(blueprint_text)
    ...
    assert not missing_in_code, (
        "Active spec docs missing in src/contracts/spec_versions.py SPEC_VERSIONS mapping: "
        f"{missing_in_code}"
    )
