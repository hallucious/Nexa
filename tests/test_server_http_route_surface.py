...
def test_workspace_shell_payload_exposes_role_aware_action_availability() -> None:
    ...
    assert payload['return_use_continuity_section']['return_path_kind'] == 'first_success_prerequisite'
    assert payload['return_use_continuity_section']['current_step_id'] == 'complete_first_success'
    assert any(line.startswith('Current path: First-success prerequisite') for line in payload['return_use_continuity_section']['summary']['lines'])
...
def test_public_sdk_catalog_route_returns_broader_sdk_export_surface() -> None:
    response = RunHttpRouteSurface.handle_public_sdk_catalog(
        http_request=HttpRouteRequest(method="GET", path="/api/integrations/public-sdk/catalog"),
    )

    assert response.status_code == 200
    assert response.body["status"] == "ready"
    assert response.body["catalog"]["surface_family"] == "public-sdk-catalog"
...
def test_public_mcp_manifest_route_returns_manifest_export_surface() -> None:
    ...
    assert response.body["routes"]["public_hub_page"] == "/app/public"
    assert response.body["routes"]["public_integration_hub_page"] == "/app/integrations"
...
def test_launch_workspace_shell_route_rejects_when_required_provider_binding_is_missing() -> None:
    response = RunHttpRouteSurface.handle_launch_workspace_shell(
        http_request=_auth_request(method='POST', path='/api/workspaces/ws-001/shell/launch', path_params={'workspace_id': 'ws-001'}, json_body={'input_payload': {'question': 'hello from provider shell'}}),
        workspace_context=_workspace(),
        workspace_row={'workspace_id': 'ws-001', 'owner_user_id': 'user-owner', 'title': 'Primary Workspace', 'description': 'Main'},
        artifact_source=_provider_backed_working_save('ws-shell-provider-001'),
        provider_binding_rows=(),
        managed_secret_rows=(),
        provider_probe_rows=(),
    )
    ...
    assert response.status_code == 409
    assert response.body['reason_code'] == 'launch.provider_binding_missing'
    assert response.body['blocking_findings'][0]['provider_key'] == 'openai'
