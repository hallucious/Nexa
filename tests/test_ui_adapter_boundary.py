...
def test_ui_adapter_routes_product_flow_gateway_through_stable_boundary() -> None:
    adapter = NexaUIViewAdapter(
        latest_working_save=_working_save(),
        latest_commit_snapshot=_commit(),
        latest_execution_record=_run(),
    )
    gateway_vm = adapter.read_product_flow_gateway_view_model(
        _working_save(),
    ...
    assert gateway_vm.source_role == "working_save"
    assert gateway_vm.current_gateway_id in {"review", "commit", "run", "followthrough"}
...
def test_ui_adapter_routes_product_flow_readiness_through_stable_boundary() -> None:
    ...
    readiness_vm = adapter.read_product_flow_readiness_view_model(
        _working_save(),
        validation_report=_validation_report(),
        execution_record=_run(),
        session_state_card=_session_card(),
    ...
    assert readiness_vm.source_role == "working_save"
    assert readiness_vm.current_boundary_id == "commit"
...
def test_ui_adapter_routes_product_flow_runbook_through_stable_boundary() -> None:
    adapter = NexaUIViewAdapter(
        latest_working_save=_working_save(),
        latest_commit_snapshot=_commit(),
        latest_execution_record=_run(),
    )
    runbook_vm = adapter.read_product_flow_runbook_view_model(
        _working_save(),
        validation_report=_validation_report(),
        execution_record=_run(),
        session_state_card=_session_card(),
