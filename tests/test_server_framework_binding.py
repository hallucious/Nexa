...
def test_framework_binding_handles_public_plugin_catalog_round_trip() -> None:
    response = FrameworkRouteBindings.handle_public_plugin_catalog(
        request=_request(method="GET", path="/api/integrations/public-plugins/catalog"),
    )
...
def test_framework_binding_workspace_shell_includes_latest_run_previews() -> None:
    ...
    assert parsed["latest_run_result_preview"]["result_summary"] == "Success."
    ...
    assert 'Summary: Success.' in parsed['latest_run_result_detail']['items']
