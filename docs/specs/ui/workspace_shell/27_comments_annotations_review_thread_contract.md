# Comments / Annotations / Review Thread Contract v1

## Recommended save path
`docs/specs/ui/workspace_shell/27_comments_annotations_review_thread_contract.md`

## 1. Purpose

This document fixes the operational contract for anchored discussion model inside the Nexa shell.
...
## 8. Three discussion objects

- comment = one utterance
- annotation = where it is anchored
- review thread = one discussion bundle

Comments must anchor to something real:
node, edge, output, diff item, finding, trace event, artifact, or explicit review context.
