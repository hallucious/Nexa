...
* governance transition rules now use anchor-aware hysteresis: elevated/strict tiers are held until an explicit anchored referential resolution occurs, and anchored success relaxes the tier one step at a time instead of dropping immediately to standard
