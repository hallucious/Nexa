...
## Step43: ExecutionTrace를 엔진 핵심 계약으로 승격 (Hard Requirement: Node Coverage)

이 Step은 Trace를 “부가 로그”가 아니라 **엔진의 공식 실행 증거(Contract)**로 확정한다.

### 핵심 결정
- Trace는 **그래프 기반**이며, Engine의 **모든 node_id**를 반드시 포함한다(미실행 포함).
- Trace/NodeTrace는 생성 후 **불변(immutable)** 이다.
- Trace는 `execution_id`, `revision_id`, `structural_fingerprint`를 반드시 포함한다.
- Trace의 Node Coverage 누락은 **실행 실패로 간주**한다.

### 근거 문서(정본)
- `docs/specs/trace_model.md` (v1.1.0)

### 구현 원칙(요약)
- Trace 생성 시점에 node coverage를 자동 검증한다(수동 호출 금지).

---

## Step44: v1 최소 실행 의미(Minimal Execution Semantics) 확정 및 Trace 반영

목적:
- 아직 Node 실행/Flow 분기/Channel 데이터 전달을 구현하기 전이라도,
  Engine.execute()가 “실행이 발생했다”는 최소 의미를 Trace에 남기도록 한다.
...
- Validation 실패 시:
  - 모든 노드는 NOT_REACHED 유지하며, validation 결과만 Trace에 기록한다.

정본 계약:
- `docs/specs/execution_model.md` (v1.1.0)
- `docs/specs/trace_model.md` (v1.1.0)
