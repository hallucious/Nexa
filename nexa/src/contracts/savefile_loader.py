...
def load_savefile(data: Dict[str, Any]) -> Savefile:
    # working_save / commit_snapshot payloads must be routed through the
    # storage layer before reaching this loader. If such a payload arrives
    # here it will fail on the required-sections check below, which is the
    # correct explicit failure mode — not a silent re-route.
