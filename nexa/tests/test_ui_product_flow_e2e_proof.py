...
from src.ui.product_flow_e2e_proof import read_product_flow_e2e_proof_view_model


def _working_save() -> WorkingSaveModel:
    return WorkingSaveModel(
        meta=WorkingSaveMeta(format_version="1.0.0", storage_role="working_save", working_save_id="ws-001", name="Proof Draft"),
...
def _validation_report(blocking: int = 0, warning: int = 1) -> ValidationReport:
    ...
    return ValidationReport(role="working_save", findings=findings, blocking_count=blocking, warning_count=warning, result=("failed" if blocking else "passed_with_findings"))
...
def test_product_flow_e2e_proof_marks_full_proof_when_live_working_save_has_review_and_completed_run_evidence() -> None:
    vm = read_product_flow_e2e_proof_view_model(
        _working_save(),
    ...
    assert vm.proof_status == "proven"
    assert vm.overall_proven is True
    assert vm.current_checkpoint_id == "followthrough"
    assert vm.recommended_checkpoint_id == "followthrough"
    assert vm.proven_checkpoint_count == 4


def test_product_flow_e2e_proof_marks_commit_gap_blocked_when_precheck_is_blocked() -> None:
    vm = read_product_flow_e2e_proof_view_model(
        _working_save(),
    ...
    assert vm.proof_status == "blocked"
    assert vm.blocked_checkpoint_count >= 1


def test_product_flow_e2e_proof_marks_live_run_checkpoint_when_execution_is_running() -> None:
    vm = read_product_flow_e2e_proof_view_model(_working_save(), execution_record=_run("running"))

    run = next(checkpoint for checkpoint in vm.checkpoints if checkpoint.checkpoint_id == "run")
