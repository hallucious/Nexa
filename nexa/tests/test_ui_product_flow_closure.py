...
def _working_save() -> WorkingSaveModel:
    return WorkingSaveModel(
        meta=WorkingSaveMeta(format_version="1.0.0", storage_role="working_save", working_save_id="ws-001", name="Closure Draft"),
...
def test_product_flow_closure_prioritizes_commit_boundary_for_reviewed_working_save() -> None:
    vm = read_product_flow_closure_view_model(
        _working_save(),
    ...
    assert vm.closure_status == "actionable"
    assert vm.current_stage_id == "commit"
    assert vm.primary_open_requirement_id == "commit_snapshot"
    assert vm.primary_recommended_flow_id == "flow:commit_snapshot"


def test_product_flow_closure_prefers_followthrough_when_execution_history_is_available() -> None:
    vm = read_product_flow_closure_view_model(_working_save(), execution_record=_run("completed"))

    assert vm.source_role == "working_save"
    assert vm.closure_status in {"actionable", "terminal"}
    assert any(stage.stage_id == "followthrough" and (stage.closeable or stage.closed) for stage in vm.stages)
    assert vm.closeable_stage_count >= 1 or vm.closed_stage_count >= 1


def test_product_flow_closure_marks_commit_boundary_blocked_when_precheck_blocks_progress() -> None:
    vm = read_product_flow_closure_view_model(
        _working_save(),
    ...
    commit_stage = next(stage for stage in vm.stages if stage.stage_id == "commit")
    assert commit_stage.stage_status == "blocked"
    assert vm.blocked_stage_count >= 1
    assert vm.primary_open_requirement_id in {"request_revision", "commit_snapshot", "approve_for_commit"}
