...
def build_fastapi_router(binding: Any) -> APIRouter:
    """Build the FastAPI route registry for a FastApiRouteBindings instance.

    This module owns route registration so src/server/fastapi_binding.py can stay
    focused on app composition, observability bootstrap, and edge middleware.
    The local `self` alias intentionally preserves the existing route-handler
    implementation while moving the route registry out of the app shell.
    """
    self = binding
    router = APIRouter()
    ...
    async def launch_workspace_shell(request: Request, workspace_id: str, payload: dict[str, Any] | None = Body(default=None)) -> Response:
        ...
        outbound = FrameworkRouteBindings.handle_launch_workspace_shell(
            ...
            file_extraction_reader=self.dependencies.file_extraction_store.get_workspace_extraction,
