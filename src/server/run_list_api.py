...
from src.server.run_read_models import ProductExecutionTargetView, ProductResultSummaryView, ProductSourceArtifactView
...
def _source_artifact_view_from_list_rows(
    run_row: Mapping[str, Any],
    result_row: Mapping[str, Any] | None = None,
) -> ProductSourceArtifactView | None:
    metrics = result_row.get('metrics') if isinstance(result_row, Mapping) and isinstance(result_row.get('metrics'), dict) else None
    ...
    target_type = str(run_row.get('execution_target_type') or '').strip() or None
    target_ref = str(run_row.get('execution_target_ref') or '').strip() or None
    ...
    row_source_ws = str(run_row.get('source_working_save_id') or '').strip() or None
        ...
        for row in page:
            ...
            items.append(
                ProductRunListItemView(
                    ...
                    source_artifact=_source_artifact_view_from_list_rows(row, result_row),
