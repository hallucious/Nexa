...
class FastApiRouteBindings:
    ...
    def build_router(self) -> APIRouter:
        ...
        async def get_public_plugin_catalog_page(request: Request) -> Response:
            ...
            payload = json.loads(outbound.body_text)
            payload["app_language"] = app_language
            payload.setdefault("routes", {})["app_catalog_page"] = app_href("public_plugin_catalog_page", app_language=app_language)
            payload.setdefault("routes", {})["community_hub_page"] = app_href("community_hub_page", app_language=app_language)
            payload.setdefault("routes", {})["ecosystem_catalog_page"] = app_href("public_ecosystem_catalog_page", app_language=app_language)
            return HTMLResponse(
        ...
        async def get_public_sdk_catalog_page(request: Request) -> Response:
            ...
            routes["app_catalog_page"] = app_href("public_sdk_catalog_page", app_language=app_language)
            routes["ecosystem_catalog_page"] = app_href("public_ecosystem_catalog_page", app_language=app_language)
            routes["community_hub_page"] = app_href("community_hub_page", app_language=app_language)
            routes["public_plugin_catalog_page"] = app_href("public_plugin_catalog_page", app_language=app_language)
            routes["public_mcp_catalog_page"] = app_href("public_mcp_catalog_page", app_language=app_language)
            routes["provider_catalog_page"] = app_href("provider_catalog_page", app_language=app_language)
            routes["public_nex_format_page"] = app_href("public_nex_format_page", app_language=app_language)
            return HTMLResponse(
        ...
        async def get_public_ecosystem_catalog_page(request: Request) -> Response:
            ...
            payload["app_language"] = app_language
            routes = payload.setdefault("routes", {})
            routes["app_catalog_page"] = app_href("public_ecosystem_catalog_page", app_language=app_language)
            routes["community_hub_page"] = app_href("community_hub_page", app_language=app_language)
            routes.setdefault("public_sdk_catalog_page", app_href("public_sdk_catalog_page", app_language=app_language))
            routes.setdefault("public_plugin_catalog_page", app_href("public_plugin_catalog_page", app_language=app_language))
            for surface_name, surface in list((payload.get("surfaces") or {}).items()):
                surface_map = dict(surface or {})
                app_route_key = ecosystem_surface_app_route_key(surface_name)
                if app_route_key is not None:
                    surface_map["app_route"] = app_href(app_route_key, app_language=app_language)
                else:
                    surface_map["app_route"] = surface_map.get("route") or routes["app_catalog_page"]
                surface.clear()
        ...
        async def get_public_mcp_catalog_page(request: Request) -> Response:
            ...
            payload = {
                ...
                "routes": {
                    ...
                    "ecosystem_catalog_page": app_href("public_ecosystem_catalog_page", app_language=app_language),
                    "community_hub_page": app_href("community_hub_page", app_language=app_language),
                    "public_nex_format_page": app_href("public_nex_format_page", app_language=app_language),
                    "provider_catalog_page": app_href("provider_catalog_page", app_language=app_language),
                },
            }
        ...
        async def get_public_provider_catalog_page(request: Request) -> Response:
            ...
            payload["routes"]["app_catalog_page"] = app_href("provider_catalog_page", app_language=app_language)
            payload["routes"]["ecosystem_catalog_page"] = app_href("public_ecosystem_catalog_page", app_language=app_language)
            payload["routes"]["community_hub_page"] = app_href("community_hub_page", app_language=app_language)
            payload["routes"]["public_sdk_catalog_page"] = app_href("public_sdk_catalog_page", app_language=app_language)
            payload["routes"]["public_mcp_catalog_page"] = app_href("public_mcp_catalog_page", app_language=app_language)
        ...
        async def get_public_nex_format_page(request: Request) -> Response:
            ...
            payload = json.loads(outbound.body_text)
            payload["app_language"] = app_language
            payload.setdefault("routes", {})["app_catalog_page"] = app_href("public_nex_format_page", app_language=app_language)
            payload["routes"]["ecosystem_catalog_page"] = app_href("public_ecosystem_catalog_page", app_language=app_language)
            payload["routes"]["community_hub_page"] = app_href("community_hub_page", app_language=app_language)
            payload["routes"]["public_sdk_catalog_page"] = app_href("public_sdk_catalog_page", app_language=app_language)
            payload["routes"]["public_mcp_catalog_page"] = app_href("public_mcp_catalog_page", app_language=app_language)
            payload["routes"]["provider_catalog_page"] = app_href("provider_catalog_page", app_language=app_language)
        ...
        async def get_public_community_hub_page(request: Request) -> Response:
            ...
            if workspace_id:
                payload["workspace_id"] = workspace_id
            payload.setdefault("routes", {})["app_hub"] = app_href("community_hub_page", app_language=app_language, workspace_id=workspace_id)
            payload["routes"]["starter_template_catalog_page"] = app_href("starter_template_catalog_page", app_language=app_language, workspace_id=workspace_id)
            payload["routes"]["public_share_catalog_page"] = app_href("public_share_catalog_page", app_language=app_language, workspace_id=workspace_id)
            payload["routes"]["public_share_catalog_summary"] = app_href("public_share_catalog_summary_page", app_language=app_language, workspace_id=workspace_id)
            payload["routes"]["public_ecosystem_catalog_page"] = app_href("public_ecosystem_catalog_page", app_language=app_language, workspace_id=workspace_id)
            payload["routes"]["public_mcp_catalog_page"] = app_href("public_mcp_catalog_page", app_language=app_language, workspace_id=workspace_id)
            payload["routes"]["public_plugin_catalog_page"] = app_href("public_plugin_catalog_page", app_language=app_language, workspace_id=workspace_id)
            for asset in list(payload.get("assets") or []):
                asset_map = dict(asset or {})
                route_key = community_asset_app_route_key(str(asset_map.get("asset_family") or ""))
                if route_key is not None:
                    asset_map["app_route"] = app_href(route_key, app_language=app_language, workspace_id=workspace_id)
                asset.clear()
