...
def _baseline_update_recommendation(run_dir: Path) -> Tuple[bool, str]:
    ...
    if not baseline_present:
        return True, "Baseline missing. Recommend establishing baseline after a stable PASS (controlled update)."
    return False, "Baseline present or not applicable."
...
def gate_g7_final_review(ctx: GateContext) -> GateResult:
    ...
    baseline_reco, baseline_msg = _baseline_update_recommendation(run_dir)
    ...
    decision_md = (
        ...
        + f"- recommend_update: {baseline_reco}\n"
        + f"- note: {baseline_msg}\n"
    )
    ...
    return GateResult(
        ...
        outputs=outputs,
        meta={"hard_fail": hard_fail, "fail_hints": fail_hints, "baseline_reco": baseline_reco},
    )
