...
class GateBlueprint:
    ...
    fallback_executor: Optional[Callable[[GateContextLike], GateResult]] = None


class GateOrchestrator:
    """Builds executable gate callables from GateBlueprints."""

    def build(self, bp: GateBlueprint) -> Callable[[GateContextLike], GateResult]:
        def _exec(ctx: GateContextLike) -> GateResult:
    ...
    def _stop_with_artifacts(self, ctx: GateContextLike, gate_prefix: str, reason: str) -> GateResult:
        outputs = self._write_standard_artifacts(
            run_dir=Path(ctx.run_dir),
