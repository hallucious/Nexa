...
class AnthropicMessagesAdapter(ProviderAdapter):
    ...
    def stream(self, payload: Dict[str, Any]) -> Iterator[StreamChunk]:  # type: ignore[override]
        """Foundation streaming path.

        Native SSE parsing is intentionally deferred. For now, when a caller
        explicitly requests streaming through the adapter boundary, the adapter
        safely falls back to a regular request and yields a single truthful
        chunk derived from the final response.
        """
